...
function parseFilters(sp: SP): SearchFilters {
  ...
  return {
    ...
    scope: (["us", "intl", "all"].includes(s("scope") ?? "") ? s("scope") : "us") as SearchFilters["scope"],
...
const SCOPES: { value: string; label: string }[] = [
  ...
  { value: "all", label: "Worldwide (US + intl)" },
];
    ...
    <>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        ...
        <form method="GET" className="flex items-center gap-2">
          ...
          <label htmlFor="scope" className="text-xs text-fg-muted">
            Region
          </label>
          <select
            id="scope"
            name="scope"
            defaultValue={filters.scope ?? "us"}
