...
import { ArrowRight, AudioLines, Bookmark, Building2, ClipboardCheck, Database, History, ShieldCheck, Sparkles } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      {/* Platform areas — Phases 3–5 */}
      <div className="anim-fade-up delay-2 mt-8 grid gap-4 md:grid-cols-3">
        {[
          {
            icon: Building2, href: "/sandbox", title: "Enterprise sandbox",
            body: "Your private AVL: ingest any legacy list, search it with the network, request audits.",
          },
          {
            icon: AudioLines, href: "/meetings", title: "Meeting recommender",
            body: "Record a project meeting — get every need matched to vendors, with reasons.",
          },
          {
            icon: ClipboardCheck, href: "/inspections", title: "Inspection marketplace",
            body: "Take vendors to Level 1 Certified with any approved inspector.",
          },
        ].map((t) => (
          <Link key={t.href} href={t.href} className="card card-hover group p-5">
            <div className="flex items-center justify-between">
              ...
              <ArrowRight size={15} className="text-fg-muted transition-transform group-hover:translate-x-1 group-hover:text-arc" />
            </div>
            <h2 className="mt-3 font-display text-base font-semibold text-fg">{t.title}</h2>
            <p className="mt-1 text-xs leading-relaxed text-fg-secondary">{t.body}</p>
          </Link>
        ))}
      </div>
