...
import { CreditCard, KeyRound, LogOut, Palette, UserRound } from "lucide-react";
import { getSession } from "@/lib/auth";
import { logoutAction, changePasswordAction } from "@/lib/actions";
import { ActionForm } from "@/components/action-form";
import { Badge, Button, ButtonLink, Input, Label } from "@/components/ui";
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-8 space-y-5">
        ...
        <section className="card anim-fade-up delay-2 p-6">
          <h2 className="mb-4 flex items-center gap-2.5 font-display text-base font-semibold text-fg">
            <KeyRound size={16} className="text-arc" /> Password
          </h2>
          {session.mustChangePassword && (
            <p className="mb-4 rounded-lg border border-warn/30 bg-warn/10 px-3 py-2 text-xs text-warn">
              Your password was set by an administrator — change it now to keep using your account.
            </p>
          )}
          <ActionForm action={changePasswordAction} submitLabel="Update password" size="sm" variant="secondary">
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div>
                ...
                <Input id="new_password" name="new_password" type="password" required minLength={10}
                  placeholder="10+ characters" autoComplete="new-password" />
              </div>
              <div>
                <Label htmlFor="confirm_password">Confirm new</Label>
